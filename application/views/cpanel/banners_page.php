          ...
          <div class="box box-primary">
            ...
            <div class="box-body">
            	<?php if(isset($bannerdetails) && count($bannerdetails) != 0){ ?>
					<form role="form" method="post" action="<?=base_url('cpanel/dashboard/banners/saveEditdetails')?>" enctype="multipart/form-data">
				...
				<?php }else{ ?>
					<form role="form" method="post" action="<?=base_url('cpanel/dashboard/banners/uplaod')?>" enctype="multipart/form-data">
					  <div class="box-body">
						<div class="form-group">
						  <label for="exampleInputFile">Upload Banner <span class="text-danger">*</span></label>
						  <input type="file" id="exampleInputFile" name="BannersImages" accept=".png,.jpg,.jpeg" required="">
				  ...
				  <tbody>
					<?php $i = 1; foreach ($banners as $key => $banner) { ?>
					  <tr>
						...
						<td>
						  <a href="<?=base_url('cpanel/dashboard/banners/'.$banner->id.'/edit')?>" onclick="return confirm('Are you sure to edit banner details..?')"><i class="fa fa-edit"></i></a>&nbsp;
