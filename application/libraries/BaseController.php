	...
	**/
	...
	function isLoggedIn() {
		...
		if (!isset($isLoggedIn) || $isLoggedIn != TRUE) {
			...
			$userdata = $this->session->userdata('loginuserdata');
	...
	function isOnlineLoggedIn() {
		$isOnlineLoggedIn = $this->session->userdata('isOnlinelogin');
		if (!isset($isOnlineLoggedIn) || $isOnlineLoggedIn != TRUE) {
			redirect(base_url('users/loginaccount'));
		}else{
			$userdata = $this->session->userdata('onlineuserdata');
	      	/*$this->id 			= $this->session->userdata('id');
	      	$this->regid 		= $this->session->userdata('regid');
	      	$this->type 		= $this->session->userdata('type');
	      	$this->isLoggedIn 	= $this->session->userdata('isLoggedIn');
	...
	//auto load view (header and footer)
  	function loadViews($viewName = "", $pagedata = NULL){
		$this->load->view('header', $pagedata);
		$this->load->view($viewName, $pagedata);
		$this->load->view('footer', $pagedata);
  	}

  	function manualLoadView($header,$body,$footer,$pagedata = NULL){
		$this->load->view($header, $pagedata);
		$this->load->view($body, $pagedata);
		$this->load->view($footer, $pagedata);
  	}
