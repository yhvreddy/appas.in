...
class Reviews extends BaseController {
	...
	public function index(){
		...
		$data['reviews']	=	$this->Model_default->select_data('meat_products_reviews',array('status <='=>1),'updated ASC');
	...
	public function publishReview(){
		...
		if(isset($type) && !empty($type) && $type == 'unpublish'){
			$update = $this->Model_default->update_data('meat_products_reviews',array('status'=>0,'updated'=>date('Y-m-d H:i:s')),array('id'=>$id));
			$message = 'unpublish';
		}else{
			$update = $this->Model_default->update_data('meat_products_reviews',array('status'=>1,'updated'=>date('Y-m-d H:i:s')),array('id'=>$id));
			$message = 'publish';
		}
