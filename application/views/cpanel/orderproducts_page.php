          ...
          <div class="box box-info">
            ...
            <div class="box-body">
              ...
              <table class="table table-striped table-bordered datatable">
                ...
                <tbody>
                  <?php $i = 1; foreach ($orders as $key => $order) { //print_r($order); ?>
                    <tr>
                      <td><?=$i++;?></td>
                      ...
                      <td>
                        ...
                        <?php if($order->order_status == 1){ ?>
                          <a href="<?=base_url('orders/'.$order->id.'/3/updatestatus')?>" class="btn btn-sm btn-success" onclick="return confirm('You want to update order status..!');">Ordered -> Ready</a>
                        <?php }else if($order->order_status == 2){ ?>
                          <a href="#" class="btn btn-sm btn-danger">Failed By user</a>
                        <?php }else if($order->order_status == 3){ ?>
                          <a href="<?=base_url('orders/'.$order->id.'/4/updatestatus')?>" class="btn btn-sm btn-warning" onclick="return confirm('You want to update order status..!');">Ready -> Delivered</a>
                        <?php }else if($order->order_status == 4){ ?>
                          <a href="#" class="btn btn-sm btn-success">Delivered</a>
                        <?php }else{ ?>
                          <a href="#" class="btn btn-sm btn-danger">Order Failed</a>
                        <?php } ?>    

                        <a href="#" data-backdrop="static" data-keyboard="false" data-toggle="modal" data-target="#myModal_<?=$order->id?>" class="btn btn-sm btn-success"><i class="fa fa-files-o"></i></a>
                      </td>
                    </tr>
                  <?php } ?>
                </tbody>
